# Book 1 — Story Bible: The Case of the Missing Hot Sauce

The canonical source of truth for Book 1. Story-level facts live here. Series-level decisions live in `canon library/series_plan.md`. Where this file and the chapter prose disagree, the prose wins (Golden Rule) and this file is updated to match.

---

## Identity

| Field | Value |
|---|---|
| **Title** | The Case of the Missing Hot Sauce |
| **Series** | Supper Club Secrets (Book 1 of 6) |
| **Focal character** | Emma Hartley |
| **Month** | October (2026 calendar; anchor below) |
| **Host** | Emma (all four meals) |
| **Genre** | Cozy crime |
| **Status** | v1 complete. 25 chapters drafted (the three short Meal 4 road chapters promoted to full chapters 19/21/22, 2026-07-05). Prose locked on the opaque-secret / economic-squeeze direction; canon & tracking reconciled 2026-07-05. Only optional craft-polish items remain. See [Status & Open Issues](#status--open-issues). |

---

## Logline

When a beloved hot sauce vendor vanishes from the Brooklyn farmers market, a supper club of six friends investigates over weekly dinners and uncovers a corporate land grab that turns personal when the developer targets one member's livelihood to force her to back off.

---

## Premise

Emma Hartley, three weeks out from quitting a tyrannical restaurant kitchen, goes to the McGolrick Park farmers market and finds that Hank, her favorite hot sauce vendor, has vanished. Dorothy, the neighboring honey vendor, hands Emma Hank's last bottle with a note that his great-grandmother "would've wanted her peppers shared." Emma cooks with the sauce, the supper club tastes it, and the disappearance becomes personal for the whole group.

Investigating one friend per day, they assemble a single picture: a developer is using shell companies and bureaucratic and legal pressure to clear out artisan vendors. The case turns dangerous when an impulsive call tips off the developer's people, who retaliate by squeezing Emma's livelihood: assignments get cancelled, her income dries up, and legal pressure mounts, all designed to make her drop it. The group fractures, then regroups around a coordinated counter-campaign while Jasper drives to Pennsylvania to find Hank. They turn public sentiment, the developer's investors pull out, and the threat collapses. The first case is solved, and the supper club, without quite meaning to, becomes a thing that fixes things.

## Themes

Trusting your instincts even when they have failed you before. Found family and the strength of a group that covers each other's blind spots. Local community versus extractive capital. Shame and exposure transformed into solidarity.

---

## Structure

Four meals over roughly three weeks, mapped to Save the Cat beats: Setup, Investigation, Crisis, Resolution.

| Meal | Beat | Chapters | Count |
|---|---|---|---|
| Meal 1 | Setup | 1–5 (`m1_*`) | 5 |
| Meal 2 | Investigation | 6–12 (`m2_*`) | 7 |
| Meal 3 | Crisis | 13–17 (`m3_*`) | 5 |
| Meal 4 | Resolution | 18–25 (`m4_*`) | 8 |

**Counts (descriptive, not targets):** 25 chapters, ~38,600 words of chapter prose in the current draft. Chapter count is not a design target; the three short Meal 4 road chapters (19, 21, 22) are the deliberate brief-chapter device. See [Reading Experience & Development Approach](#reading-experience--development-approach).
**Calendar anchor:** Week 1 Sunday = 2026-10-04. Final dinner = Sunday 2026-10-25. Full day-by-day in `tracking/timeline_ledger.md`.

### Chapter list (one-line summaries)

**Meal 1 — Setup**
1. *The Missing Hot Sauce* — Emma finds Hank's stall empty; Dorothy gives her his last bottle.
2. *The First Supper* — The club tastes the soup; the disappearance becomes personal.
3. *A Simmering Suspicion* — Marcus's text ("a man running from something") raises the stakes.
4. *Stirring the Pot* — Theory competition; Elijah judges and can't pick a winner.
5. *Mise en Place* — The plan: investigate and reconvene with a potluck. Assignments drawn.

**Meal 2 — Investigation** (one character per day, Mon–Sat)
6. *A Fine Sieve* — Oliver at NYPL discovers LLC #2847.
7. *Gathering the Ingredients* — Olivia gets intel from Marcus, pulls the LLC out of Oliver at home.
8. *The Bottom Line* — Elijah bridges the find to the group chat; builds the financial model privately.
9. *A Pinch Too Much* — Noah traces the corporate shell structure.
10. *Loose Lips* — Jasper calls Brenda Marquez and inadvertently tips off the developer.
11. *The Empty Stall* — Emma at McCarren market (stall replaced) and Sofia's shop; the "big offer."
12. *Coming to a Boil* — Potluck; findings converge; false victory. Jasper misses Brenda's warning.

**Meal 3 — Crisis**
13. *Service for One* — Silence from contacts; the cease and desist arrives at Emma's.
14. *The Seamless Supper* — Emergency takeout; the group rallies around the C&D; Emma privately confides her precarity to Olivia.
15. *A Bitter Taste* — "How did they find us?" Jasper confesses; the fight; Oliver's speech.
16. *The Recipe for Revenge* — The swarm-strategy plan; roles assigned; Jasper leaves to find Hank.
17. *Sharpening the Knives* — Preparation vignettes; each member readies their piece, alone but connected.

**Meal 4 — Resolution**
18. *Turning Up the Heat* — Campaign launches; small blogs pick it up; hope builds.
19. *The Ridgeline Diner* — From the road: Jasper's first night of nursery calls; dead ends and one flicker of resolve.
20. *Ghost Kitchen* — Pushback and doubt; the campaign might fail.
21. *The Starlite Drive-In* — From the road: Noah's spreadsheet narrows the search; "meadow-something" surfaces.
22. *Murph's* — From the road: the low point in a dive bar, where a bartender finally names the Meadowlight Collective.
23. *Farm to Table* — Jasper's solo road trip; finds Hank at the Meadowlight Collective.
24. *The Proof* — Oliver's rebuttal breaks through; a food critic amplifies it.
25. *Family Meal* — Celebratory dinner; Jasper arrives with Hank; the club commits to continuing.

---

## Focal Arc — Emma

Moves from defensive secrecy to openness. Her instinct that "Big Hot Sauce" is behind the disappearance, played for laughs early, turns out to be essentially correct. She learns to trust the group and her own judgment. Her Western PA dialect surfaces under stress as the tell for when her polished chef persona drops.

## The B-Story: Emma's Financial Precarity

Emma's financial vulnerability is the B-plot. She has a private, more reliable income source, but it stays **opaque throughout Book 1**: the reader and the group are never shown what it is.

The developer's retaliation lands here. Rather than exposing anything, the pressure arrives as **lost assignments and dried-up revenue**, an economic squeeze meant to make Emma back off. Emma's **on-page vulnerability** is her shame over the gap between her proud, dramatic exit from Bistro Lavande and the precarious reality of freelance life, and that is what she **confides to Olivia** privately (not the whole group). The secret income source sits opaquely beneath that, **deferred to a later book**, where it can be revealed properly.

*Supersedes the earlier plan in which a cease and desist exposed an OnlyFans account ("Kinky Kitchen"), Emma confessed to the entire group in Ch14, and it went public in Ch18. The underlying canon fact (the secret itself) is unchanged in `canon library/characters/emma.md`; only its Book-1 revelation changes. See the 2026-06-15 changelog entry for the downstream files to reconcile in the back-half rewrite.*

## Central Mystery

**What happened:** Hank gave up his stalls and disappeared under pressure from a developer assembling a waterfront restaurant play through shell LLCs (notably LLC #2847), squeezing artisan vendors out via bureaucratic and legal intimidation.

**The villain:** Garrett Pike ("the mogul"). Self-interested, not evil. His investors pull out after the campaign; his plan stalls rather than being destroyed (he may resurface).

**Resolution:** A coordinated public-sentiment campaign plus Oliver's rebuttal document and a food critic's amplification flips the narrative. The C&D is withdrawn. Hank returns from the Meadowlight Collective in Pennsylvania, where Jasper found him.

---

## Cast

### Supper club (mains)

| Character | One-line role in Book 1 |
|---|---|
| **Emma Hartley** | Focal POV; chef who finds the mystery and is personally targeted. |
| **Olivia** | The group's social connector and intel-gatherer; fierce defender of Emma. |
| **Oliver** | Quiet researcher; finds the LLC; delivers the rally speech and the rebuttal. |
| **Elijah** | Guarded finance mind; connective tissue; builds the financial model. |
| **Noah** | Tech skeptic turned essential; traces the shell structure; builds campaign infrastructure. |
| **Jasper** | Chaos agent whose call causes the breach; redeems himself by finding Hank. |

### Minor & recurring characters

| Character | Role in Book 1 | Future |
|---|---|---|
| **Hank** | The missing vendor. Returns; files a trademark. | Cameos; returns Book 6 (market reopening). |
| **Dorothy** | Honey vendor; gives Emma the bottle. | Central in Book 3 (loan crisis). |
| **Sofia** | Cheese shop owner; source of the "big offer" lead. | Central Book 2; returns Book 5. |
| **Marcus** | The Gilded Fern owner; intel source via Olivia. | Returns Book 4; possible Book 2. |
| **Brenda Marquez** | Recognizes the LLC; inadvertently tips off the developer. | Returns Book 6; possible Book 3. |
| **Ruth** | Vendor who notes Hank's last jittery appearance. | Available for callbacks. |
| **The Food Critic** | Unnamed; amplifies the story using Oliver's framework. | Available for callbacks. |

### Antagonist

**Garrett Pike** ("the mogul"). Full profile in `canon library/antagonists/book1_garrett_pike.md`.

---

## Key Locations

Emma's apartment (Williamsburg, near McGolrick Park; all four meals). McGolrick Park Farmers Market (Sunday; where the mystery starts). McCarren Park Greenmarket (Saturday). The Gilded Fern (Marcus's boutique). Sofia's Cheese Shop. NYPL Rose Main Reading Room (Oliver's research). Meadowlight Collective, PA (where Hank is found; the only setting outside Brooklyn). Full registry with schedule rules in `canon library/locations_registry.md`.

---

## Series Position

**Reputation status:** The group does not yet think of itself as mystery-solvers. This was a one-time thing. (Per the series reputation arc, Books 1–2 are "stumbling into it.")

**Seeds planted for later books:** Oliver's secret online life (Book 2 focal). Dorothy's finances (Book 3). Elijah's family weight, named but unpaid here by design (Book 3 focal). Jasper's old money, undisclosed (Book 6 reveal). Emma's secret income source stays opaque; full reveal deferred to a later book. Rotating hosts begin Book 2. Full continuity in `canon library/continuity.md`.

---

## Reading Experience & Development Approach

**Target:** quick, crushable reads. Chapter count is not a target or a constraint; chapters run as long or short as the pace wants. Brief chapters are a deliberate device for momentum, used especially in the Meal 3 crisis, where short, fast beats tighten tension.

**How we develop and evaluate Book 1 (top-down, three tiers).** Get the structure sound before polishing prose. Work down the tiers, and resolve a problem found at a higher tier before descending.

1. **High level** — does the premise, focal arc, mystery logic, and four-meal shape hold together?
2. **Meal level** — does each meal's arc, emotional state, beat coverage, and pace work?
3. **Chapter level** — individual scenes, prose, voice, and continuity.

---

## Status & Open Issues

**v1 complete (2026-07-05).** All chapters are drafted, including Ch20 and Ch24 and the three Meal 4 mini-chapters. The back-half revision to the opaque-secret / economic-squeeze direction is done, and the canon library + tracking files were reconciled to the finalized prose on 2026-07-05 (see `CANON_CHANGELOG.md`).

Remaining items are optional polish and housekeeping, not blockers:

- **Craft flags (tracking/):** a few editorial "action needed" notes remain in `tracking/subplot_threads.md` and `tracking/character_matrix.md` (e.g., an active Emma beat in Meal 4, Sofia's silence payoff, letting the Ch14 Olivia confide breathe). Judgment calls, not continuity errors.
- **Compression notes:** several earlier flags (Ch12→13, Ch17 montage) are largely resolved by the finalized prose; `tracking/timeline_ledger.md` still lists them for a final read-through.
- **(resolved 2026-07-05)** Ch20 "Ghost Kitchen" and Ch24 "The Proof" are the canonical titles (the cooking-metaphor prose titles win over the old outline names "The Counterpunch" / "The Turning Tide"); files, metadata, and outline reconciled.
- **Held by design:** Elijah's family-weight thread is intentionally ambient in Book 1 with no payoff (reserved for Book 3).
- **Deferred:** Emma's secret income source stays opaque; pick the book/occasion for its eventual reveal (per `series_plan.md`).
