# Supper Club Secrets

## Overview
*Supper Club Secrets* is a "cozy crime" anthology series centered around a group of friends in their late 20s and early 30s navigating life in Brooklyn. Each story follows the group as they uncover and solve strange, local mysteries, one dinner party at a time.

**Target Audience:** Adults 18-40 seeking "crushable" reads—engaging enough to hold attention, satisfying enough to finish in 2-3 sittings.

## Core Concept
The central premise of the series is that each mystery is investigated and unraveled through the group's weekly supper club gatherings. These dinners serve as the main setting for exposition, brainstorming, and planning. Each character brings their unique skills and perspectives to the table, contributing to the solution in their own way.

## Setting
The series is set in modern-day **Williamsburg, Brooklyn**. The neighborhood itself plays a significant role, with its mix of trendy shops, diverse communities, and local secrets providing a rich backdrop for the mysteries.

## Main Characters
*   **Emma:** The grounded and creative chef.
*   **Elijah:** The rigid and pragmatic analyst.
*   **Jasper:** The chaotic and well-connected wildcard.
*   **Noah:** The idealistic and tech-savvy engineer.
*   **Oliver:** The fastidious and bookish introvert.
*   **Olivia:** The social butterfly and neighborhood networker.

## Themes
*   The power of friendship and community.
*   The challenges of "coming of age" in your late 20s and early 30s.
*   The tension between tradition and modernity.
*   The idea that anyone can be a detective.

---

## Structure & Format

### Book Length
**45,000-50,000 words / ~180-200 pages** per book.

### Story Architecture
Each book is structured around **four dinner gatherings** (meals), mapping to a modified Save the Cat beat sheet:

| Meal | Story Phase | Key Beats |
|------|-------------|-----------|
| **Meal 1** | Setup | Discovery, debate, catalyst |
| **Meal 2** | Investigation | Fun & games, midpoint revelation |
| **Meal 3** | Crisis | All is lost, dark night |
| **Meal 4** | Resolution | Climax, celebration |

### Host Rotation
- **Book 1:** All meals at Emma's apartment (establishing the tradition)
- **Books 2+:** Rotate hosts—one character hosts per book, or rotate per meal

For detailed writing guidance, see [04_writing_guide.md](04_writing_guide.md).

---

## Anthology Guidelines

### Standalone Mysteries
Each book should work independently. Readers can start anywhere in the series.

### Recurring Elements
- The supper club and its traditions
- Character relationships and ongoing arcs
- Williamsburg setting and recurring locations
- Light callbacks to past mysteries (never essential)

### Series Arc
While mysteries are self-contained, characters evolve across books:
- Relationships deepen or strain
- Secrets are slowly revealed
- Individual arcs pay off over 3-5 books