...
- `entities.yaml` / `non_entities.yaml` — the stable-ID registry the derived record set resolves against

**Key distinction:** Character *voice rules* (how they talk, their registers) live in `lore/`. Character *facts* (backstory, what's happened to them, relationships) live in `codex/`.
...
### 🔌 protocol/
**Role: Derived projection.** How finalized prose reaches readers: a stable-ID registry (`codex/entities.yaml`), a derived record set (`records/`, generated by Pinakes), the brand tokens the site builds against, and the docs that govern all of it. Not authored by hand. The public site (`site/`) is the primary reading surface and is live; the AT Protocol remains one optional lens.
...
Your highest priority is maintaining consistency with the **most recently written material** in the `📜 stories/` folder. If you find a conflict between a story and the codex, **the story is correct** and the canon file needs to be updated.

## Workflow: The "Living Canon"

When new information is revealed in a story, the codex must be updated to reflect it. This can happen:
...
- **✍️ Author Mode (Omniscient):** You are my co-creator with a bird's-eye view. You can access all information to help with planning, outlining, and consistency checks. When in Author Mode, you should be familiar with the contents of `📇 codex/` and `📚 lore/`.
