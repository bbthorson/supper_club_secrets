...
- *Negative parallelism (3.1).* Trimmed **four** genuinely reflexive uses: Ch1's "Not just because X … but because Y" opener; Ch4's "Not a theory, exactly" (the same construction as Ch5's better-earned toast beat); Ch12's "Not physically—" opener; Ch18's third standalone "Not yet." The earned uses stay — Oliver's Ch15 "Not for Emma. Not for Hank. Not for Paolo," Noah's Ch14 "Not to me. Not to Oliver," Emma's Ch12 "Not the technique, not the plating," Ch1's "Not repurposed, not filled by another vendor" — all of which are anaphora doing rhetorical work or dialogue in a character's voice.
  **Counting note (corrected 2026-09-17, after author challenge):** a fifth edit in the same batch — Dorothy's "Not like him at all," four lines after her own "Not like him" — is **not** this device. It is a flat judgment with no corrective replacement; the real defect was phrase repetition inside one speech. The edit stands on its own merits but does not count here. The first pass also reported a book-wide count of "42 → 34," which came from a grep for sentence-initial `Not ` and over-counts: it sweeps in participials ("Not wanting to hear Jasper and Noah argue"), conditionals ("Not unless you had a plan"), and dialogue punchlines ("Not after the 2nd Avenue Debacle"). **Classified by hand, the true count is ~31 before this pass and ~27 after — four removed, not eight** — which is roughly where the 07-12 audit's own mechanical count (32 → 27) already had the book. Future passes should classify rather than grep. The audit's guidance was that the device "doesn't need to hit zero," and it remains well short of a tic.
...
**Author decisions, same day:** multi-chapter same-day drops approved; the surface split decided in principle — the book/community handle publishes prose, and **each character is their own AT Protocol identity** (own PDS repo for their lexicon records, ideally with authored in-character posts). The doc gained a full "Character accounts" section: domain handles from the codex `handle:` fields, record routing by `subject`, posts as a new *authored-and-linted* canon surface (never live-generated; knowledge-state checked; fiction disclosure on every account non-negotiable; the swarm campaign is not re-enacted with real accounts), Pike's-side artifact accounts, the optional Oliver pseudonymous-account easter egg, and three build tiers. Scene records publish as bare index cards (`beat`/`primaryEvent` stay internal) per the author's "publishing too narrowly" reframe. Still open: the Oct 2 teaser (pending author review), which tier ships Oct 1, domain, Hank/Dorothy accounts.

**Second iteration, same day:** cast accounts are **declared bots** per the atproto bot tutorial (profile self-label `bot` + fiction bio; opt-in interaction only — which settles the reply-policy question). Domain purchase identified as the launch-blocking task (handles derive from it). On the author's idea of putting the core six's posts in an **AT Protocol Space**: reconciled with §12.6 via the register model — *public-register posts stay on the public lane* (a space is invisible to the Bluesky app and not web-public; readers need an atproto identity + our admitted app), while the space's right cargo is the *private register*: the **supper club group chat**, authored now as derivable `message` records (single-writer topology — project DID with in-record character attribution — avoiding the no-relay syncer), published only when spaces leave alpha ("full launch later this year" per the announcement; the alpha PDS runs no backups and may destructively migrate, so nothing reader-facing rides it for the immovable Oct 1 launch). New space type NSID to reserve: `site.supperclub.groupchat`.

**Third iteration, same day:** author confirmed the split (public posts on the public lane; group chat in the space; requiring an atproto identity for the full experience is acceptable) and settled the account roster: **core six only** — no Hank, Dorothy, or other side characters, because the space *is* the club's group chat and they wouldn't invite outsiders in (space membership = club membership; readers are publisher-credentialed flies on the wall, not chat members). Still open: Oct 2 teaser, which tier ships Oct 1, domain choice, Tier C artifact accounts, single-writer sign-off, and alpha-wait vs. ride-the-alpha for the space lane.
