...
**Revision, same day (author direction):** the book now opens on **Olivia**, not Emma — she's running a one-day pop-up of thrifted glassware from a folding table in Sofia's shop when the inspector comes in and writes the notice. The scene is deliberately small (he's polite, Sofia is annoyed rather than frightened), and it ends with Olivia telling Sofia the Hank story and volunteering the group, because "Oliver will love this." That single line is now the book's engine: she hands her husband the thing that takes him out of her orbit, cheerfully, in the first five pages, which gives her Book 4 anxiety an origin instead of only a symptom. Emma's promised Book 1 Ch25 shop visit survives as Ch2 the same afternoon — two feeds, one Saturday, establishing the POV pattern immediately. Free clue upgrade: Sofia asks where Danny is, the district inspector of nine years, so the reassignment mechanism is spoken aloud on page two and dismissed. Chapter count 24 → 25 (6+7+5+7); everything from the old Ch2 on shifted by one and all cross-references were renumbered.
