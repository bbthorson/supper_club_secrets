#!/usr/bin/env python3
"""
Resolution sanity check (Task D) — Entity Registry, Protocol Phase 1.

Proves that EVERY distinct name used across Book 1's 25 chapters' frontmatter
...
resolve against characters (and items). This keeps the character "Sofia"
distinct from the place "Sofia's Cheese Shop".
...
def parse_registry(path):
    ...
    Understands the grouped-list shape this repo uses: top-level group keys
...
def extract_names(fm_lines):
    """Return dict field -> list of (raw_value) for the fields we resolve."""
    ...
    for line in fm_lines:
        ...
        if m_reg and in_registers:
            found["registers"].append(m_reg.group(1).strip())
            continue
    return found
...
def main():
    ...
    chapter_files = sorted(glob.glob(os.path.join(CHAPTERS, "m*.md")))
    for path in chapter_files:
        fm = read_frontmatter_lines(path)
        names = extract_names(fm)
        ch = os.path.basename(path)
        buckets = [
            ("pov", "place" if False else None, names["pov"]),
        ...
        # pov holds character names (or a "Distributed …" montage marker)
        buckets[0] = ("pov", "character", names["pov"])
    ...
    # ---- Report ----
    print("=" * 70)
    print("ENTITY RESOLUTION SANITY CHECK — Book 1 chapter frontmatter corpus")
    print("=" * 70)
